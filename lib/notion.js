...
const statusProp = (s) => (s ? { status: { name: s } } : { status: null })
...
export function pageToOrder(page) {
  ...
  // Debug: log raw Size and Quantity from Notion
  if (p['Size'] || p['Quantity']) {
    console.log('[notion] Size raw:', JSON.stringify(p['Size']))
    console.log('[notion] Quantity raw:', JSON.stringify(p['Quantity']))
  }
  return {
    id:                  page.id,
    ...
    aedPrice:            getNumber(p['AED Prices']),
    city:                getText(p['City']),
...
export async function orderToProps(notion, order) {
  ...
  if (order.phone               !== undefined) props['Phone']                = richText(order.phone)
  if (order.size                !== undefined) props['Size']                 = selectProp(order.size)
  if (order.quantity            !== undefined) props['Quantity']             = numProp(order.quantity)
  if (order.aedPrice            !== undefined) props['AED Prices']            = numProp(order.aedPrice)
  if (order.city                !== undefined) props['City']                  = richText(order.city)
